...
import { AtSign, Globe, ExternalLink, Pencil } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { formatCategory } from "@/lib/constants";
import type { Profile } from "@/types/database";

interface MakerProfileViewProps {
  profile: Profile;
  isOwner?: boolean;
}

export function MakerProfileView({ profile, isOwner = false }: MakerProfileViewProps) {
        ...
        <div>
          <div className="flex flex-wrap items-center gap-3">
            <h1 className="text-4xl font-semibold tracking-tight text-stone-800">
              {profile.business_name || "Untitled Maker"}
            </h1>
            {profile.category && (
              <Badge variant="accent">{formatCategory(profile.category)}</Badge>
            )}
          </div>
      ...
      <section>
        ...
        {profile.portfolio_images.length > 0 ? (
          <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-4">
            {profile.portfolio_images.map((url, index) => (
              <div
                key={`${url}-${index}`}
              ...
              >
                ...
                <img
                  src={url}
                  alt={`${profile.business_name} portfolio ${index + 1}`}
          ...
          <div className="rounded-xl border border-dashed border-stone-200 bg-white/60 px-6 py-16 text-center">
            ...
            {isOwner && (
              <Button variant="outline" className="mt-4" asChild>
                <Link href="/profile/edit">Add portfolio images</Link>
              </Button>
            )}
          </div>
        )}
